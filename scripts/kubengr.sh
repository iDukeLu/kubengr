...
print_join_command() {
    output=$(kubeadm token create --print-join-command --ttl=0)
    ip_port=$(echo "$output" | awk '{print $3}')
    token=$(echo "$output" | awk '{print $5}')
    cert_hash=$(echo "$output" | awk '{print $7}')

    echo_green "join worker by: "
    echo_green "    kubengr join --host-name <node-name> --master-address $ip_port --token $token --discovery_token_ca_cert_hash $cert_hash"
}
